	...
	if(code==-1) {
		...
		for(int i=0;i<DPlay->GetMaxPlayers();i++) {
			if(PlayerData[i].ReceiveData.info.dpnidPlayer==playerInfo->dpnidPlayer) {
				...
				PlayerData[i].rtime2=PlayerData[i].rtime2+PlayerData[i].span2+1; //とりあえず1足しとく
				DWORD timeGt=frameGetTime;
				if(timeGt-PlayerData[i].rtime>1000){ //ﾛｰｶﾙで計算した相手の内部時間の積算誤差が60を超えたまたは0未満の時ﾘｾｯﾄ
		...
		for(int i=0;i<DPlay->GetMaxPlayers();i++) {
			if(PlayerData[i].ReceiveData.info.dpnidPlayer==playerInfo->dpnidPlayer) {
				...
				PlayerData[i].rtime2=PlayerData[i].rtime2+PlayerData[i].span2+1; //とりあえず1足しとく
				DWORD timeGt=frameGetTime;
				if(timeGt-PlayerData[i].rtime2>1000){ //ﾛｰｶﾙで計算した相手の内部時間の積算誤差が60を超えたまたは0未満の時ﾘｾｯﾄ
					PlayerData[i].rtime2=timeGt;
				}
				PrePlayerData[i].time2=PlayerData[i].time2;
