...
GFloat GWorld::CalcJoint(GRigid* rigidA, GVector &offsetA, GRigid* rigidB, GVector &offsetB,GFloat friction)
{
	...
	*/
	GVector	b = (rigidB->V + pB.cross(rigidB->W)) - (rigidA->V + pA.cross(rigidA->W)); //VとWは古い値だからまずくないの?
	GVector	c  = (rigidB->P * rigidB->M_ + (rigidB->L * rigidB->I_).cross(pB))
		- (rigidA->P * rigidA->M_ + (rigidA->L * rigidA->I_).cross(pA)) ; //P*M_はVだしL*I_はWだし、ここで計算すべきじゃなくね?
	GVector a=wB - wA;
	GFloat s;
