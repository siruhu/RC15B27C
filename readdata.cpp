...
void setOption(GRigid *rigid,GFloat value)
{
	...
	if(rigid->ChipType==GT_ARM) {	//ARM
		...
		rigid->Option=value;
		rigid->Density=1000.0f*0.7f*4*value;
		rigid->Reset();
		...
		while(cw!='}') {
			...
			while(cw!=')') {
				...
				if(strcmp("DEFAULT",str)==0) {
					if(!checkFlag) ValList[VarCount].Def=(GFloat)temp;
				}
				else if(strcmp("MIN",str)==0) {
					if(!checkFlag) ValList[VarCount].Min=(GFloat)temp;
				}
				else if(strcmp("MAX",str)==0) {
					if(!checkFlag) ValList[VarCount].Max=(GFloat)temp;
				}
				else if(strcmp("STEP",str)==0) {
					if(!checkFlag) ValList[VarCount].Dec=(GFloat)temp;
				}
				else if(strcmp("DISP",str)==0) {
					if(!checkFlag) ValList[VarCount].visible=(int)temp;
				}
		...
		while(cw!='}') {
			...
			while(cw!=')') {
				...
				if(strcmp("STEP",str)==0) {
					if(!checkFlag){
						sscanf(str3,"%lf",&v);
						KeyList[n].Step[KeyList[n].Count]=(GFloat)v;
					}
				}
	...
	if((fp=fopen(fname,"w"))!=NULL) {
		...
		for(int i=0;i<VarCount;i++) {
			if(ValList[i].Label==0) {
				fprintf(fp,"\t%s(default=%f, step=%f"
					,ValList[i].Name,ValList[i].Def,ValList[i].Dec);
				if(ValList[i].Max<FLT_MAX) fprintf(fp,", max=%f",ValList[i].Max);
				if(ValList[i].Min!=0) fprintf(fp,", min=%f",ValList[i].Min);
		...
		for(int i=0;i<GKEYMAX;i++) {
			if(KeyList[i].Count>0) {
				...
				for(int j=0;j<KeyList[i].Count;j++) {
					if(j>0) fprintf(fp,", "); 
					fprintf(fp,"%s(step=%f)",ValList[KeyList[i].ValNo[j]].Name,KeyList[i].Step[j]);
				}
				fprintf(fp,"\n");
			}
		}
